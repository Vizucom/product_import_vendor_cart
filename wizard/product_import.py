...
class ProductImportWizard(models.TransientModel):
    ...
    def validate_contents(self):
        ...
        for mapping in self.vendor_settings_id.field_mapping_ids:
            ...
            if file_format == 'csv':
                if mapping.column_name not in [header_string.decode('UTF-8') for header_string in header_row]:
        ...
        for col_index in range(column_range):
            if file_format == 'csv':
                cell_header = header_row[col_index].decode('UTF-8')
            else:
        ...
        for row_number in range(range_start, range_stop):
            ...
            for mapping in self.vendor_settings_id.field_mapping_ids:
                if mapping.product_field_ids:
                    ...
                    for product_field_id in mapping.product_field_ids:
                        if product_field_id.id == self.vendor_settings_id.identifying_field_id.id:
                            identifying_field_value = self.get_cell_value(file_format, row_number, mapping.column_number - 1, excel_data=sheet, csv_data=csv_rows)
                ...
                if mapping.supplierinfo_field_ids:
                    for supplierinfo_field_id in mapping.supplierinfo_field_ids:
                        ...
                        if supplierinfo_field_id.ttype == 'many2one' and supplierinfo_field_id.relation == 'res.currency':
                            search_term = self.get_cell_value(file_format, row_number, mapping.column_number - 1, excel_data=sheet, csv_data=csv_rows)
                ...
                if mapping.product_field_ids:
                    for product_field_id in mapping.product_field_ids:
                        if product_field_id.ttype == 'many2one':
                            ...
                            # Other types of fields (char, text, int, float) can have the field contents added with no special handling
                            product_vals[product_field_id.name] = self.get_cell_value(file_format, row_number, mapping.column_number - 1, excel_data=sheet, csv_data=csv_rows)
                ...
                if mapping.supplierinfo_field_ids:
                    for supplierinfo_field_id in mapping.supplierinfo_field_ids:
                        if supplierinfo_field_id.ttype == 'many2one' and supplierinfo_field_id.relation == 'res.currency':
                            ...
                            supplierinfo_vals[supplierinfo_field_id.name] = self.get_cell_value(file_format, row_number, mapping.column_number - 1, excel_data=sheet, csv_data=csv_rows)
